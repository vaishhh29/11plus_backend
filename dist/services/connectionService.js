...
class ConnectionService {
     ...
     */
    static async joinTeacher(userId, teacherCode) {
        const student = await database_1.default.studentProfile.findUnique({
            where: { userId },
        });
        if (!student) {
            const error = new Error('Student profile not found. Only students can perform this action.');
            error.statusCode = 403;
            throw error;
        }
        const teacher = await database_1.default.teacherProfile.findUnique({
            where: { teacherCode },
        });
        if (!teacher) {
            const error = new Error('Invalid teacher code. Teacher not found.');
            error.statusCode = 404;
            throw error;
        }
        // Check if already linked
        const existingLink = await database_1.default.studentTeacher.findUnique({
            where: {
                studentId_teacherId: {
        ...
        await database_1.default.studentTeacher.create({
            data: {
                studentId: student.id,
     ...
     */
    static async linkChild(userId, studentCode) {
        const parent = await database_1.default.parentProfile.findUnique({
            where: { userId },
        });
        if (!parent) {
            const error = new Error('Parent profile not found. Only parents can link children.');
            error.statusCode = 403;
            throw error;
        }
        const student = await database_1.default.studentProfile.findUnique({
        ...
        if (!student) {
            const error = new Error('Invalid student code. Child profile not found.');
            error.statusCode = 404;
            throw error;
        }
        ...
        const updatedParent = await database_1.default.parentProfile.findUnique({
            where: { id: parent.id },
            include: {
                students: {
                    select: {
     ...
     */
    static async requestTeacher(userId, studentId, teacherId) {
        const parent = await database_1.default.parentProfile.findUnique({
            where: { userId },
        });
        if (!parent) {
        ...
        const student = await database_1.default.studentProfile.findFirst({
            where: { id: studentId, parentId: parent.id },
        });
        if (!student) {
            const error = new Error('Unauthorized. The student must be linked to your profile.');
            error.statusCode = 403;
            throw error;
        }
        ...
        // Check if already linked
        const alreadyLinked = await database_1.default.studentTeacher.findUnique({
            where: {
                studentId_teacherId: {
                    studentId,
                    teacherId,
        ...
        if (alreadyLinked) {
            const error = new Error('This student is already connected to this teacher.');
            error.statusCode = 400;
            throw error;
        }
        // Check if pending request exists
        const existingRequest = await database_1.default.teacherRequest.findFirst({
            where: {
                studentId,
                teacherId,
        ...
        const request = await database_1.default.teacherRequest.create({
            data: {
                studentId,
                studentName: student.name,
                grade: student.grade,
    ...
    static async getPendingRequests(userId) {
        ...
        const requests = await database_1.default.teacherRequest.findMany({
            where: {
                teacherId: teacher.id,
                status: client_1.RequestStatus.PENDING,
            },
     ...
     */
    static async respondToRequest(userId, requestId, status) {
        const teacher = await database_1.default.teacherProfile.findUnique({
            where: { userId },
        });
        if (!teacher) {
        ...
        const request = await database_1.default.teacherRequest.findUnique({
            where: { id: requestId },
        });
    ...
    static async getAllTeachers() {
        return database_1.default.teacherProfile.findMany({
            select: {
                id: true,
                teacherCode: true,
                name: true,
     ...
     */
    static async getParentPendingRequests(userId) {
        const parent = await database_1.default.parentProfile.findUnique({
            where: { userId },
        });
        if (!parent) {
